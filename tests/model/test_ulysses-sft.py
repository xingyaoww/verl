...
def debug_print(msg, data):
    ...
    color = "green" if rank == 0 else "magenta"
    if rank == 1:
        return
    print(colored(msg, color))
    if isinstance(data, dict):
        for name, tensor in data.items():
            print(colored(f"{name}: {tensor.shape}", color))
    else:
        print(colored(str(data.shape), color))
...
def test_hf_casual_fwd_bwd():
    ...
    for model_name, (config, attn) in test_configs.items():
        ...
        debug_print("Original Inputs", model_inputs)

        model_inputs = DataProto.from_dict(model_inputs)

        # 1. perform ulysses forward
        with sharding_manager:
            # NOTE: STEP 1: ulysses_preprocess_data 
            # It simply all gather the model_inputs to each sp group
            # previously, each worker have n unique data points,
            # now, each worker have global view of n*sp_size data points
            ...
            # NOTE: STEP 2: standard unpad_input
            # This is used to remove the padding tokens from the input_ids and position_ids
            # This is used to align the input_ids and position_ids to the flash attention varlen
            ...
            before_ulysses_pad_and_slice_inputs = {
                "input_ids": input_ids_rmpad,
                "position_ids": position_ids_rmpad
            }
            # debug_print("Before Ulysses Pad and Slice Inputs", before_ulysses_pad_and_slice_inputs)

            # NOTE: STEP 3: ulysses_pad_and_slice_inputs
            input_ids_rmpad_sliced, position_ids_rmpad_padded, pad_size = ulysses_pad_and_slice_inputs(
                input_ids_rmpad, position_ids_rmpad, sp_size=get_ulysses_sequence_parallel_world_size())

            after_ulysses_pad_and_slice_inputs = {
                "input_ids": input_ids_rmpad_sliced,
                "position_ids": position_ids_rmpad_padded
            }
            debug_print("After Ulysses Pad and Slice Inputs", after_ulysses_pad_and_slice_inputs)
            ...
            # debug_print("Logits Split in Seq", logits_split_in_seq)

            # NOTE: STEP 4: gather_outpus_and_unpad
            logits_full = gather_outpus_and_unpad(logits_split_in_seq, gather_dim=1, unpad_dim=1, padding_size=pad_size)
            # debug_print("Logits Full", logits_full)
